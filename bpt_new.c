...
void find_and_print_int(node * root, int key, bool verbose);
node * find_leaf( node * root, char * key, bool verbose );
record * find( node * root, char * key, bool verbose );
record * find_int( node * root, int key, bool verbose);
int cut( int length );
...
node * insert_int( node * root, int key, int value);
...
node * delete_int( node * root, int key);
...
void find_and_print_int(node * root, int key, bool verbose) {
	...
	record * r = find_int(root, key, verbose);
	if (r == NULL)
...
record * find_int( node * root, int key, bool verbose) {
	...
	record * p = find(root, key_str, verbose);
	free(key_str);
	return p;
}
...
node * make_node( void ) {
	...
	new_node->keys = malloc( (order - 1) * sizeof(char *) );
...
node * insert_into_leaf( node * leaf, char * key, record * pointer ) {
	...
	leaf->keys[insertion_point] = malloc(key_length);
...
node * insert_into_leaf_after_splitting(node * root, node * leaf, char * key, record * pointer) {
	...
	temp_keys = malloc( order * sizeof(char *) );
	if (temp_keys == NULL) {
	...
	temp_keys[insertion_index] = malloc(key_length);
...
node * insert_into_node(node * root, node * n, 
	...
	n->keys[left_index] = malloc(key_length);
...
node * insert_into_node_after_splitting(node * root, node * old_node, int left_index, 
	...
	temp_keys[left_index] = malloc(key_length);
...
node * insert_int( node * root, int key, int value) {
	...
	memset(key_str, 0, key_length);
	sprintf(key_str, "%d", key);
	node * p = insert(root, key_str, value);
	free(key_str);
	return p;
}
...
node * coalesce_nodes(node * root, node * n, node * neighbor, int neighbor_index, char * k_prime) {
	...
	if (!n->is_leaf) {
		...
		neighbor->keys[neighbor_insertion_index] = malloc(key_length);
...
int main( int argc, char ** argv ) {
	...
	if (argc > 2) {
		...
		while (!feof(fp)) {
			fscanf(fp, "%d\n", &input);
			root = insert_int(root, input, input);
		}
		fclose(fp);
		print_tree(root);
	}
	...
	while (scanf("%c", &instruction) != EOF) {
		...
		case 'p':
			scanf("%d", &input);
			find_and_print_int(root, input, instruction == 'p');
			break;
		case 'l':
			print_leaves(root);
			break;
